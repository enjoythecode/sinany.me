...
Cell.prototype = {
	...
	,execute: function(engine) {
		...
		var build_length = build_keys.length;
		var build_current = 0;
		...
		if(Type.typeof(result) == ValueType.TInt) {
			return result;
		}
		return null;
	}
...
CoinFlipper.__super__ = Building;
...
Engine.prototype = {
	...
	,can_action: function(act) {
		var blg = null;
		if(act == "buy1") {
			blg = Defs.S_BLG_FLIPPER;
		}
		if(act == "buy2") {
			blg = Defs.S_BLG_TV;
		}
		if(act == "buy3") {
			blg = Defs.S_BLG_3THIRD;
		}
		if(blg == null) {
			return false;
		}
...
GameLoop.prototype = {
	...
	,draw_initial: function() {
		...
		while(blg_current < blg_length) {
			...
			container.id = "building-" + blg.shortName + "-container";
...
Main.__name__ = true;
Main.main = function() {
	haxe_Log.trace("Entropy?! by sunny",{ fileName : "src/Main.hx", lineNumber : 6, className : "Main", methodName : "main"});
	var gl = new GameLoop();
};
Math.__name__ = true;
...
UnnamedThirdBuilding.__super__ = Building;
